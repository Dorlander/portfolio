...
function verifyOnlyOfficeCallbackSignature(req, body) {
  if (!OO_JWT_SECRET) {
    // без секрета подлинность callback проверить нечем: в production
    // отклоняем (иначе любой в сети мог дёрнуть callback), в dev пропускаем
    if (process.env.NODE_ENV === 'production') {
      return { ok: false, error: 'ONLYOFFICE_JWT_SECRET is not set' };
    }
