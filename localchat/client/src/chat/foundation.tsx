 ...
 */
// URL в тексте сообщения становятся кликабельными ссылками (только
// http/https, в новой вкладке). Композируется с подсветкой поиска:
// сначала режем по ссылкам, к текстовым кускам применяем поиск.
// URL до пробела/угловой скобки; хвостовую пунктуацию (.,;:!?) и парные
// закрывающие скобки отрезаем ниже, чтобы «…акт).» не попало в ссылку
const URL_RE = /(https?:\/\/[^\s<>]+)/g;
const trimTrailingPunct = (url: string): [string, string] => {
  let u = url;
  let tail = '';
  // отрезаем висящую пунктуацию и непарные закрывающие скобки
  while (u.length > 0) {
    const last = u[u.length - 1];
    if ('.,;:!?»"\''.includes(last)) { tail = last + tail; u = u.slice(0, -1); continue; }
    if (last === ')' && (u.match(/\(/g)?.length ?? 0) < (u.match(/\)/g)?.length ?? 0)) {
      tail = last + tail; u = u.slice(0, -1); continue;
    }
    break;
  }
  return [u, tail];
...
export function messageBodyWithLinks(body: string, searchQ?: string): ReactNode {
  ...
  for (const m of body.matchAll(URL_RE)) {
    const idx = m.index ?? 0;
    if (idx > last) {
      const text = body.slice(last, idx);
      parts.push(searchQ ? <span key={`t-${k++}`}>{messageBodyWithSearchMarks(text, searchQ)}</span> : text);
    }
    const raw = m[0];
