...
// известные заглушки из репозитория — с ними production стартовать нельзя
const KNOWN_DEFAULT_SECRETS = [DEV_JWT_SECRET, 'docker-dev-change-jwt-secret'];
...
if (process.env.NODE_ENV === 'production') {
  ...
  if (KNOWN_DEFAULT_SECRETS.includes(JWT_SECRET)) {
    console.error('[FATAL] JWT_SECRET не должен совпадать с известным значением по умолчанию (dev/docker).');
    process.exit(1);
  }
  if (JWT_SECRET.trim().length < 16) {
    console.error('[FATAL] JWT_SECRET слишком короткий — используйте случайную строку от 32 символов.');
    process.exit(1);
  }
}
