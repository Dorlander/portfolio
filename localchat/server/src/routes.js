...
export function createApiRouter(io) {
  ...
  // Публичный конфиг для клиента: адрес производственного учёта УТК
  // (кнопка «Учёт УТК» в сайдбаре). Задаётся через env UCHET_URL.
  r.get('/public/config', (_req, res) => {
    res.json({ uchetUrl: (process.env.UCHET_URL || '').trim() || null });
  });
